...
from .ruleset import Stage
from .http import HttpUA
...
class TestRunner(object):
    ...
    def test_header(self, response_object, regex):
        """
        Checks if the response header contains a regex specified in the
        output stage. It will assert that the regex is present.
        """
        if response_object is None:
            raise errors.TestError(
        ...
        headers_str = "\n".join([key + ": " + response_object.headers.get(key, "") for key in response_object.headers.keys()])
        # assert regex == False
        if regex.search(ensure_str(headers_str, errors="ignore")):
            assert True
        else:
            assert False
    ...
    def run_stage(self, stage: Stage, logger_obj: LogChecker = None, http_ua: HttpUA = None) -> Tuple[bool, str]:
        ...
        if stage.output.header_contains_str:
            self.test_header(http_ua.response_object, stage.output.header_contains_str)
        if stage.output.response_contains_str:
